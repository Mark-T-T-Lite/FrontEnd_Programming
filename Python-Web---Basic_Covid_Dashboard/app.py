...
################################################################################
# Generate similar line graphs for Dead and Recovered and show it on the Dashboard alongside the confirmed trend
# Generate logarithmic graph for confirmed cases for all countries in one plot
# Write a function to download latest data at a set frequency
# Deploy this to a web server which can be accessed wordwide
...
#Get the Data (Maintained by John Hopkins University)
...
#Prepare the data in cleaned time series format for a chosen country
def process_data(data,cntry='Uganda',window=3):
    conf_ts = data
...
#Get Overall worldwide total for confirmed recovered, dead cases
...
#Get total for confirmed, recovered, dead, for country
def get_cntry_total(df,cntry='Uganda'):
...
def fig_world_trend(cntry='Uganda',window=3):
    ...
    fig.update_layout(title_x=0.5,plot_bgcolor='#000000',paper_bgcolor='#000000',font={'color':'#00ff00'},xaxis_title="Date",yaxis_title=yaxis_title)
    return fig
...
def get_page_heading_subtitle():
    return html.Div(children='Current COVID-19 Data for A country of choice.\n(Source maintained by John Hopkins University)',
                                         style={
                                             'textAlign':'center',
                                             'color': '#FFFFFF'
                                         })
...
def get_country_dropdown(id):
    return html.Div([
                        html.Label('Select Country',style={'color':"#00ff00"}),
...
def graph1():
    return dcc.Graph(id='graph1',figure=fig_world_trend('Uganda'))
...
def generate_cards(cntry='Uganda'):
    conf_cntry_total = get_cntry_total(covid_conf_ts,cntry)
    dead_cntry_total = get_cntry_total(covid_dead_ts,cntry)
    recv_cntry_total = get_cntry_total(covid_recv_ts,cntry)
    cards = html.Div(
        [
            dbc.Row(
                [
                    ...
                    dbc.Col(dbc.Card(generate_card_content("Dead",dead_cntry_total,dead_overall_total),color="danger", inverse=True),md=dict(size=2)),
                ],
                className="mb-4",
            ),
        ],id='card1'
...
def get_slider():
    return html.Div([  
                        ...
                        html.Div([html.Label('Select Moving Average Window')],id='my-div'+str(id),style={'textAlign':'center','color':"#00ff00"})
                    ])
